...
class AlignmentVAE(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        loss_record = {
            ...
            "emb_loss_st": H['st']['emb_loss'] if H['st'] is not None else 0,
            "emb_loss_sm": H['sm']['emb_loss'] if H['sm'] is not None else 0,
        }
    ...
    def fit_vae(
        ...
        for epoch in range(1, max_epoch+1):
            ...
            for b, (X_st, X_sm) in enumerate(zip(X_train_st, X_train_sm)):
                ...
                kldiv_loss_st = kl_weight * L['kldiv_loss_st'].mean()
                kldiv_loss_sm = kl_weight * L['kldiv_loss_sm'].mean()
                emb_loss_st = L['emb_loss_st']
                emb_loss_sm = L['emb_loss_sm']

                loss = (
                    reconstruction_loss_sm.mean() * weight_sm +
                    reconstruction_loss_st.mean() * weight_st + 
                    kldiv_loss_st * weight_st +
                    kldiv_loss_sm * weight_sm + 
                    emb_loss_st * weight_st +
                    emb_loss_sm * weight_sm 
                )
                ...
                avg_kldiv_loss = (kldiv_loss_st + kldiv_loss_sm) / n_per_batch
    ...
    def get_quantitized_indices(
        ...
        for b, (X_st, X_sm) in enumerate(zip_longest(X_train_st, X_train_sm)):
            ...
            if H['st'] is not None:
                Zs_st.append(H['st']['info'][2].detach().cpu().numpy())
            if H['sm'] is not None:
                Zs_sm.append(H['sm']['info'][2].detach().cpu().numpy())
