...
class ConditionalVAEAtt2(ConditionalVAE):
    ...
    def encode(
        ...
        #var_sm = self.z_var_fc_single(q_sm).exp() + eps
        #mu_st = self.z_mean_fc_single(q_st)
        #var_st = self.z_var_fc_single(q_st).exp() + eps
        ...
        q = q_sm * beta_sm.unsqueeze(1) +  q_st * beta_st.unsqueeze(1)
        q_mu = self.z_mean_fc_single(q)
        q_var = torch.exp(self.z_var_fc_single(q)) + eps
        z = Normal(q_mu, q_var.sqrt()).rsample()
        
        H = dict(
            st = dict(
                q = q_st,
                q_mu = self.z_mean_fc_single(q_st),
                q_var = torch.exp(self.z_var_fc_single(q_st))+eps,
                beta = beta_st,
            ),
            sm = dict(
                q = q_sm,
                q_mu = self.z_mean_fc_single(q_sm),
                q_var = torch.exp(self.z_var_fc_single(q_sm))+eps,
                beta = beta_sm,
            ),
            #q = q,
            q_mu = q_mu,
        ...
        for epoch in range(1, max_epoch+1):
            ...
            for b, X in enumerate(X_train):
                ...
                loss = avg_reconstruction_loss_sm + \
                    ...
                    avg_mmd_loss  + (avg_kldiv_loss_sm *kl_weight) + (avg_kldiv_loss_st *kl_weight)
