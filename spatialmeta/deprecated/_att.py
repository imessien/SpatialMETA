...
class ConditionalVAEAtt(ConditionalVAE):
    ...
    def encode(
        ...
        q_var = self.z_var_fc_single(q).exp() + eps
        z = Normal(q_mu, q_var.sqrt()).rsample()
        '''
        H = dict(
            st = dict(
                ...
                q_mu = mu_st,
                q_var = var_st,
                z = Normal(mu_st, var_st.sqrt()).rsample(),
                beta = beta_st,
            ),
            sm = dict(
                q = q_sm,
                q_mu = mu_sm,
                q_var = var_sm,
                z = Normal(mu_sm, var_sm.sqrt()).rsample(),
                beta = beta_sm,
            ),
            #q = q,
            q_mu = q_mu,
    ...
    def fit_core(self,
        ...
        for epoch in range(1, max_epoch+1+epoch_pre):
            self._trained = True
            ...
            for b, X in enumerate(X_train):
                ...
                if epoch < epoch_pre:
                    loss = avg_reconstruction_loss_sm_corr + \
