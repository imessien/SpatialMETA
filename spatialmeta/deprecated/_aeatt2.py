...
class ConditionalAEAtt2(ConditionalVAE):
    ...
    def encode(
        ...
        '''
        g_sm = torch.exp(torch.matmul(self.to_k(q_sm), self.v))
        g_st = torch.exp(torch.matmul(self.to_k(q_st), self.v))
        ...
        self.w_omega = Parameter(
            torch.FloatTensor(self.n_hidden, self.n_hidden)
        ).to(self.device)
        
        self.u_omega = Parameter(torch.FloatTensor(self.n_hidden, 1)).to(self.device)
        
        q = torch.cat([
            torch.unsqueeze(torch.squeeze(q_sm), dim=1), 
            torch.unsqueeze(torch.squeeze(q_st), dim=1), 
        ], dim=1).to(self.device)
        
        v = F.tanh(torch.matmul(q, self.w_omega))
        vu=  torch.matmul(v, self.u_omega)
        alpha = F.softmax(torch.squeeze(vu) + 1e-6)
        #print(alpha.shape)
        q_combined = torch.matmul(torch.transpose(q,1,2), torch.unsqueeze(alpha, -1))
        q_combined = torch.squeeze(q_combined)
        q_mu = self.z_mean_fc_single(q_combined)
        # chage q_combined dim to (batch_size, n_latent)
        H = dict(
            st = dict(
                q = q_st,
                q_mu = mu_st,
    ...
    def forward(
        ...
        if self.reconstruction_method_sm == 'zg':
            ...
            reconstruction_loss_sm = (reconstruction_loss_sm * (X_SM > 0).float()).sum(dim=1)
            ...
            reconstruction_loss_sm_corr = (reconstruction_loss_sm_corr * (X_SM > 0).float()).sum(dim=1)
