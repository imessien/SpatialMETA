...
class VQVAE(BaseVAE):
    ...
    def encode(
        ...
        q_mu_st = self.z_mean_fc(q_st)
        q_var_st = torch.exp(self.z_var_fc(q_st)) + eps
        
        q_mu_sm = self.z_mean_fc(q_sm)
        q_var_sm = torch.exp(self.z_var_fc(q_sm)) + eps
        
        z_st = Normal(q_mu_st, q_var_st.sqrt()).rsample()
        z_sm = Normal(q_mu_sm, q_var_sm.sqrt()).rsample()
        
        z_vq_st, emb_loss_st, info_st = self.quantize(
            einops.rearrange(
                z_st,
                '(b w h) d -> b d w h', 
                w = 1,
        ...
        z_vq_sm, emb_loss_sm, info_sm = self.quantize(
            einops.rearrange(
                z_sm,
                '(b w h) d -> b d w h', 
                w = 1,
